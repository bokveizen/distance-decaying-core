...
void nodeRemoval(int v_rem, int k_global, int k_local) {
    ...
    auto slow_nodes = all_remaining_vec;
    bool use_parallel = (HAVE_OPENMP && slow_nodes.size() >= TH_OPENMP_SLOW);
    #if HAVE_OPENMP
...
int main(int argc, char *argv[]) {
    ...
    while (!remaining.empty()) {
        ...
        while (!nodes_to_remove.empty()) {
            // Asign coreness to nodes to remove
            for (int node : nodes_to_remove) {
                coreness[node] = cur_coreness;
            }
            // If all the nodes are going to be removed, we do early stopping
            if (nodes_to_remove.size() == remaining.size()) {
                early_stopping = true;
                break;
            }            
            // When the number of nodes_to_remove >= from_scratch_threshold * n, we do from scratch
            if (nodes_to_remove.size() >= from_scratch_threshold * remaining.size()) {
                ...
                // For each node to remove, we do nodeRemoval
                for (int node : nodes_to_remove) {
                    remaining.erase(node);
                    nodeRemoval(node, k_global, k_local);
                }
            }
